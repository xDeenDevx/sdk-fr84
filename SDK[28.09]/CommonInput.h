/********************************************************
*                                                       *
*   Package generated using UEDumper by Spuckwaffel.    *
*                                                       *
********************************************************/

/// Package CommonInput.

/// Class /Script/CommonInput.CommonUIInputData
/// Size: 0x0020 (0x000028 - 0x000048)
class UCommonUIInputData : public UObject
{ 
public:
	FDataTableRowHandle                                DefaultClickAction;                                         // 0x0028   (0x0010)  
	FDataTableRowHandle                                DefaultBackAction;                                          // 0x0038   (0x0010)  
};

/// Class /Script/CommonInput.CommonInputBaseControllerData
/// Size: 0x00D8 (0x000028 - 0x000100)
class UCommonInputBaseControllerData : public UObject
{ 
public:
	ECommonInputType                                   InputType;                                                  // 0x0028   (0x0001)  
	unsigned char                                      UnknownData00_5[0x3];                                       // 0x0029   (0x0003)  MISSED
	FName                                              GamepadName;                                                // 0x002C   (0x0008)  
	unsigned char                                      UnknownData01_5[0x4];                                       // 0x0034   (0x0004)  MISSED
	FText                                              GamepadDisplayName;                                         // 0x0038   (0x0018)  
	FText                                              GamepadCategory;                                            // 0x0050   (0x0018)  
	FText                                              GamepadPlatformName;                                        // 0x0068   (0x0018)  
	TArray<FInputDeviceIdentifierPair>                 GamepadHardwareIdMapping;                                   // 0x0080   (0x0010)  
	TWeakObjectPtr<UTexture2D*>                        ControllerTexture;                                          // 0x0090   (0x0028)  
	TWeakObjectPtr<UTexture2D*>                        ControllerButtonMaskTexture;                                // 0x00B8   (0x0028)  
	TArray<FCommonInputKeyBrushConfiguration>          InputBrushDataMap;                                          // 0x00E0   (0x0010)  
	TArray<FCommonInputKeySetBrushConfiguration>       InputBrushKeySets;                                          // 0x00F0   (0x0010)  


	/// Functions
	// Function /Script/CommonInput.CommonInputBaseControllerData.GetRegisteredGamepads
	TArray<FName> GetRegisteredGamepads();                                                                                   // [0xbc9930] Final|Native|Static|Public 
};

/// Class /Script/CommonInput.CommonInputSettings
/// Size: 0x00D8 (0x000028 - 0x000100)
class UCommonInputSettings : public UObject
{ 
public:
	TWeakObjectPtr<UClass*>                            InputData;                                                  // 0x0028   (0x0028)  
	TMap<FName, FCommonInputPlatformBaseData>          CommonInputPlatformData;                                    // 0x0050   (0x0050)  
	bool                                               bEnableInputMethodThrashingProtection;                      // 0x00A0   (0x0001)  
	unsigned char                                      UnknownData00_5[0x3];                                       // 0x00A1   (0x0003)  MISSED
	int32_t                                            InputMethodThrashingLimit;                                  // 0x00A4   (0x0004)  
	double                                             InputMethodThrashingWindowInSeconds;                        // 0x00A8   (0x0008)  
	double                                             InputMethodThrashingCooldownInSeconds;                      // 0x00B0   (0x0008)  
	bool                                               bAllowOutOfFocusDeviceInput;                                // 0x00B8   (0x0001)  
	unsigned char                                      UnknownData01_5[0x7];                                       // 0x00B9   (0x0007)  MISSED
	UClass*                                            InputDataClass;                                             // 0x00C0   (0x0008)  
	FCommonInputPlatformBaseData                       CurrentPlatform;                                            // 0x00C8   (0x0038)  


	/// Functions
	// Function /Script/CommonInput.CommonInputSettings.GetRegisteredPlatforms
	TArray<FName> GetRegisteredPlatforms();                                                                                  // [0xbc9970] Final|Native|Static|Private 
};

/// Class /Script/CommonInput.CommonInputSubsystem
/// Size: 0x00C8 (0x000030 - 0x0000F8)
class UCommonInputSubsystem : public ULocalPlayerSubsystem
{ 
public:
	unsigned char                                      UnknownData00_2[0x20];                                      // 0x0030   (0x0020)  MISSED
	FMulticastInlineDelegate                           OnInputMethodChanged;                                       // 0x0050   (0x0010)  
	int32_t                                            NumberOfInputMethodChangesRecently;                         // 0x0060   (0x0004)  
	unsigned char                                      UnknownData01_5[0x4];                                       // 0x0064   (0x0004)  MISSED
	double                                             LastInputMethodChangeTime;                                  // 0x0068   (0x0008)  
	double                                             LastTimeInputMethodThrashingBegan;                          // 0x0070   (0x0008)  
	ECommonInputType                                   LastInputType;                                              // 0x0078   (0x0001)  
	ECommonInputType                                   CurrentInputType;                                           // 0x0079   (0x0001)  
	unsigned char                                      UnknownData02_5[0x2];                                       // 0x007A   (0x0002)  MISSED
	FName                                              GamepadInputType;                                           // 0x007C   (0x0008)  
	unsigned char                                      UnknownData03_5[0x4];                                       // 0x0084   (0x0004)  MISSED
	TMap<FName, ECommonInputType>                      CurrentInputLocks;                                          // 0x0088   (0x0050)  
	unsigned char                                      UnknownData04_5[0x18];                                      // 0x00D8   (0x0018)  MISSED
	bool                                               bIsGamepadSimulatedClick;                                   // 0x00F0   (0x0001)  
	unsigned char                                      UnknownData05_6[0x7];                                       // 0x00F1   (0x0007)  MISSED


	/// Functions
	// Function /Script/CommonInput.CommonInputSubsystem.ShouldShowInputKeys
	bool ShouldShowInputKeys();                                                                                              // [0xbc9b90] Final|Native|Public|BlueprintCallable|BlueprintPure|Const 
	// Function /Script/CommonInput.CommonInputSubsystem.SetGamepadInputType
	void SetGamepadInputType(FName InGamepadInputType);                                                                      // [0xbc9b00] Final|Native|Public|BlueprintCallable 
	// Function /Script/CommonInput.CommonInputSubsystem.SetCurrentInputType
	bool SetCurrentInputType(ECommonInputType NewInputType);                                                                 // [0xbc9a70] Final|Native|Public|BlueprintCallable 
	// Function /Script/CommonInput.CommonInputSubsystem.IsUsingPointerInput
	bool IsUsingPointerInput();                                                                                              // [0xbc9a40] Final|Native|Public|BlueprintCallable|BlueprintPure|Const 
	// Function /Script/CommonInput.CommonInputSubsystem.IsInputMethodActive
	bool IsInputMethodActive(ECommonInputType InputMethod);                                                                  // [0xbc99b0] Final|Native|Public|BlueprintCallable|BlueprintPure|Const 
	// Function /Script/CommonInput.CommonInputSubsystem.GetDefaultInputType
	ECommonInputType GetDefaultInputType();                                                                                  // [0xbc9900] Final|Native|Public|BlueprintCallable|BlueprintPure|Const 
	// Function /Script/CommonInput.CommonInputSubsystem.GetCurrentInputType
	ECommonInputType GetCurrentInputType();                                                                                  // [0xbc98d0] Final|Native|Public|BlueprintCallable|BlueprintPure|Const 
	// Function /Script/CommonInput.CommonInputSubsystem.GetCurrentGamepadName
	FName GetCurrentGamepadName();                                                                                           // [0xbc9890] Final|Native|Public|BlueprintCallable|BlueprintPure|Const 
};

/// Struct /Script/CommonInput.CommonInputPlatformBaseData
/// Size: 0x0038 (0x000000 - 0x000038)
struct FCommonInputPlatformBaseData
{ 
	unsigned char                                      UnknownData00_2[0x8];                                       // 0x0000   (0x0008)  MISSED
	bool                                               bSupported;                                                 // 0x0008   (0x0001)  
	ECommonInputType                                   DefaultInputType;                                           // 0x0009   (0x0001)  
	bool                                               bSupportsMouseAndKeyboard;                                  // 0x000A   (0x0001)  
	bool                                               bSupportsGamepad;                                           // 0x000B   (0x0001)  
	FName                                              DefaultGamepadName;                                         // 0x000C   (0x0008)  
	bool                                               bCanChangeGamepadType;                                      // 0x0014   (0x0001)  
	bool                                               bSupportsTouch;                                             // 0x0015   (0x0001)  
	unsigned char                                      UnknownData01_5[0x2];                                       // 0x0016   (0x0002)  MISSED
	TArray<TWeakObjectPtr>                             ControllerData;                                             // 0x0018   (0x0010)  
	TArray<UClass*>                                    ControllerDataClasses;                                      // 0x0028   (0x0010)  
};

/// Struct /Script/CommonInput.InputDeviceIdentifierPair
/// Size: 0x0018 (0x000000 - 0x000018)
struct FInputDeviceIdentifierPair
{ 
	FName                                              InputDeviceName;                                            // 0x0000   (0x0008)  
	FString                                            HardwareDeviceIdentifier;                                   // 0x0008   (0x0010)  
};

/// Struct /Script/CommonInput.CommonInputKeySetBrushConfiguration
/// Size: 0x00A8 (0x000000 - 0x0000A8)
struct FCommonInputKeySetBrushConfiguration
{ 
	TArray<FKey>                                       Keys;                                                       // 0x0000   (0x0010)  
	FSlateBrush                                        KeyBrush;                                                   // 0x0010   (0x0098)  
};

/// Struct /Script/CommonInput.CommonInputKeyBrushConfiguration
/// Size: 0x00B0 (0x000000 - 0x0000B0)
struct FCommonInputKeyBrushConfiguration
{ 
	FKey                                               Key;                                                        // 0x0000   (0x0018)  
	FSlateBrush                                        KeyBrush;                                                   // 0x0018   (0x0098)  
};

/// Enum /Script/CommonInput.ECommonInputType
/// Size: 0x05
enum ECommonInputType : uint8_t
{
	ECommonInputType__MouseAndKeyboard                                               = 0,
	ECommonInputType__Gamepad                                                        = 1,
	ECommonInputType__Touch                                                          = 2,
	ECommonInputType__Count                                                          = 3,
	ECommonInputType__ECommonInputType_MAX                                           = 4
};

